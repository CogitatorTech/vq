...
fn test_scalar_quantizer_on_scalars() {
    ...
    for x in test_values {
        ...
        // Pass the internal data slice, not the whole vector.
        let indices = quantizer.quantize(&vec_x.data);
        ...
        let clamped = if x < quantizer.min {
            quantizer.min
        } else if x > quantizer.max {
            quantizer.max
        } else {
            x
        };
...
fn test_scalar_quantizer_on_large_vectors() {
    ...
    for vector in data.iter() {
        // Again, pass the internal data slice.
        let indices = quantizer.quantize(&vector.data);
        assert_eq!(
            indices.len(),
            dim,
